...
**Total parameters: 53** (12 VictronSystem + 41 MPCTunables)
...
| Field | Default | Range | Unit | Where Set | Used In | Effect | Example |
|-------|---------|-------|------|-----------|---------|--------|---------|
| `overnight_hold_reward` | 0.10 | 0.02-0.20 | $/kWh | Number Entity `mpc_overnight_hold_reward` | `utils.scale_overnight_hold_reward()`, `optimizer.py` objective: reward for SoC at morning boundary | Base reward for maintaining SoC at end of overnight period (morning). Scaled by the arbitrage spread between overnight and morning prices: full reward when spread >= `overnight_arbitrage_threshold`, zero when spread <= 0. **Increase**: stronger overnight preservation, battery held for morning spike risk. **Decrease**: allows overnight discharge if prices justify it. | At $0.10 base with overnight avg price $0.12 and morning avg price $0.26 (spread $0.14 > $0.10 threshold), full $0.10 hold reward applies. Battery is strongly preserved because morning refill would cost more. |
...
| `morning_start_hour` | 6 | 0-23 | hour | Not yet in UI | `utils.scale_overnight_hold_reward()` (defines morning refill window start) | Start of the morning refill window used to compute the overnight-to-morning arbitrage spread. Battery held overnight is compared against what it would cost to refill from the grid during this window. | At hour 6, the window captures early-morning grid prices before solar production begins. |
| `morning_end_hour` | 9 | 0-23 | hour | Not yet in UI | `utils.scale_overnight_hold_reward()` (defines morning refill window end) | End of the morning refill window. Average buy price across `morning_start_hour` to `morning_end_hour` determines whether holding overnight was worthwhile. | At hour 9, the window ends once solar is typically generating, so the comparison reflects genuine grid dependence. |
| `overnight_arbitrage_threshold` | 0.10 | >=0 | $/kWh | Not yet in UI | `utils.scale_overnight_hold_reward()` (spread threshold for full hold reward) | Overnight-to-morning price spread at which the full hold reward applies. If overnight avg price is at least this much cheaper than morning avg, holding overnight is strongly incentivized. Below a zero spread (overnight >= morning), reward is zero. Linear interpolation between 0 and this threshold. | At threshold $0.10, a spread of $0.10 gives full $0.10 hold reward; a spread of $0.05 gives $0.05; a spread of $0.00 or negative gives $0.00. |
...
| Field | Default | Range | Unit | Where Set | Used In | Effect | Example |
|-------|---------|-------|------|-----------|---------|--------|---------|
| `genset_diesel_price_per_litre` | 2.20 | >0 | AUD/L | Not yet in UI (updated live via PetrolSpy) | `VictronSystem.genset_cost_per_kwh` (property), `coordinator.py` (live update from PetrolSpy) | Current diesel price. Updated automatically from PetrolSpy (local median based on HA location). The genset cost is used as a price floor: when grid price exceeds genset cost, the genset is the cheaper option and the LP sets its floor accordingly. | At $2.20/L: cost = (2.20 x 1.5) / 4.0 + 0.05 = $0.875/kWh. |
